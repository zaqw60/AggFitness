        ...
        <form method="post" action="{{ route('account.characteristics.update', ['characteristic' => $characteristic]) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control" name="health" id="health">
                    ...
                    <option value="A" @if ($characteristic->health === 'A') selected @endif>А – Возможны занятия физической
                        культурой без ограничений и участие в соревнованиях.</option>
                    <option value="B" @if ($characteristic->health === 'B') selected @endif>B – Возможны занятия физической
                        культурой с незначительными ограничениями физических нагрузок без
                        участия в соревнованиях.</option>
                    <option value="C" @if ($characteristic->health === 'C') selected @endif>C - Возможны занятия физической
                        культурой со значительными ограничениями физических нагрузок.</option>
                    <option value="D" @if ($characteristic->health === 'D') selected @endif>D – Возможны занятия только
            ...
            <div class="form-group">
                ...
                <textarea type="text" class="form-control" name="description" id="description" rows=6>{{ $characteristic->description }}</textarea>
                @error('description')
                    <span style="color: red">{{ $message }}</span>
                @enderror
            </div>
